...
function LmsPage() {
  ...
  useEffect(() => {
    api.list('courses').then(setData)
  }, [])

  return (
    <div>
      <h1 className="font-sans text-3xl font-bold tracking-tight text-[#191c1d]">
        Learning Hub (LMS) & Course Builder
      </h1>
      <p className="mt-1 text-sm text-[#42493e]">
        Create and manage courses, quizzes, and certifications.
      </p>
      ...
      <div className="mt-6 rounded-lg border border-[#e7e8e9] bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#42493e]">
          {data ? `Connected — ${data.items.length} courses loaded` : 'Connecting...'}
        </p>
      </div>
    </div>
  )
