...
function ConservationPage() {
  ...
  useEffect(() => {
    api.list('conservation').then(setData)
  }, [])

  return (
    <div>
      <h1 className="font-sans text-3xl font-bold tracking-tight text-[#191c1d]">
        Conservation Tracker Administration
      </h1>
      <p className="mt-1 text-sm text-[#42493e]">
        Create activity sign-ups, review evidence, and track participation.
      </p>
      ...
      <div className="mt-6 rounded-lg border border-[#e7e8e9] bg-white p-6 shadow-sm">
        ...
        <p className="mt-1 text-sm text-[#42493e]">
          {data ? `Connected — ${data.items.length} activities loaded` : 'Connecting...'}
        </p>
      </div>
    </div>
  )
